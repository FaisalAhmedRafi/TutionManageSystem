...
namespace TutionManagementSystem
{
    public partial class TutorSearch : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            string query = "select id, Name, Gender, Location, Medium, University, NumberOfDays, ExpectedSalary from Tutor where NumberOfDays = '" + numberOfDays + "' and University = '" + university + "' and Location = '" + location + "' and Medium = '" + medium + "' and Gender = '" + gender + "'";
        ...
        private void dgvTutor_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                string val = dgvTutor[e.ColumnIndex + 1, e.RowIndex].Value.ToString();
                //try
                //{
                string status = "Student";
                SqlConnection con = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=D:\\Programming\\AIUB\\C#\\TutionManagementSystem\\DB\\TutionManagementSystem.mdf;Integrated Security=True;Connect Timeout=30;");
                con.Open();
                string query = "insert into Request (Status, StudentId, TutorId) values('" + status + "', '" + id + "', '" + val + "')";
